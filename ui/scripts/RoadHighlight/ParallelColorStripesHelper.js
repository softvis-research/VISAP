const createParallelColorStripesHelper = function (controllerConfig) {
	return (function () {
        ...
        ************************/
		...
		function startRoadHighlightActionsForStartDistrict(
			startDistrictComponent,
			relatedRoadObjsMap,
		) {
		...
		function resetRoadsHighlight() {
			...
			scene.object3D.remove(
				...scene.object3D.children.filter((child) =>
					glbMeshIdArr.includes(child.uuid),
				),
			);
        ...
        ************************/
		...
		function addSectionDirections() {
			glbRelatedRoadObjsMap.forEach((roadObj, _) => {
				...
				const refDirection = getDirectionForInitialRoadSection(
					initialRoadSectionObject,
				);
				...
				for (let i = 1; i < arrLen; i++) {
					...
					const currentDirection = getDirectionOfAdjacentRoadSections(
						currentRoadSectionObj,
						refRoadSectionObj,
					);
		...
		function getDirectionForInitialRoadSection(initialRoadSectionObject) {
			const initialSectionMidPoint = document
				.getElementById(initialRoadSectionObject.id)
				.getAttribute("position");
			const startDistrictMidPoint =
				glbStartDistrictComponent.getAttribute("position");
			const directionMap = {
			...
			const refDirection = Object.keys(directionMap).find(
				(key) => directionMap[key],
			);
			return refDirection;
		}

		function getDirectionOfAdjacentRoadSections(
			currentRoadSectionObj,
			refRoadSectionObj,
		) {
			const refDirection = refRoadSectionObj.direction;
			const currentMidPoint = document
				.getElementById(currentRoadSectionObj.id)
				.getAttribute("position");
			const refMidPoint = document
				.getElementById(refRoadSectionObj.id)
				.getAttribute("position");
			// imagine a compass turning its needle based on your direction: here, assigned directions depend on ref directions
			switch (refDirection) {
				case "left":
					if (
						currentMidPoint.x > refMidPoint.x &&
						currentMidPoint.z === refMidPoint.z
					)
						return "left";
					if (
						currentMidPoint.x > refMidPoint.x &&
			...
			glbRelatedRoadObjsMap.forEach((roadObj, _) => {
				...
				if (arrLen === 1) roadObj.roadSectionObjArr[0].intersection = null;
					...
					for (let i = 1; i < arrLen; i++) {
						...
						if (
							currentRoadSectionObj.direction != refRoadSectionObj.direction
						) {
							// a curve; adding intersection coordinates
							currentRoadSectionObj.intersection,
								(refRoadSectionObj.intersection = getRoadSectionIntersection(
									roadObj,
									currentRoadSectionObj,
									refRoadSectionObj,
								));
		...
		function getRoadSectionIntersection(
			roadObj,
			currentRoadSectionObj,
			refRoadSectionObj,
		) {
			// construct two virtual stripes adjusted proportions
			const virtualStripeOfCurrent = constructVirtualStripe(
				roadObj,
				currentRoadSectionObj,
			);
			const virtualStripeOfRef = constructVirtualStripe(
				roadObj,
				refRoadSectionObj,
			);

			// intersect these stripes to get curve midpoints
			const intersection = calculateIntersectionMidpointOfTwoStripes(
				virtualStripeOfCurrent,
				virtualStripeOfRef,
			);
			return intersection;
		}
		...
		function constructVirtualStripe(roadObj, roadSectionObj) {
			...
			switch (roadSectionObj.direction) {
				case "up": {
					virtualStripe.width =
						clone.width * (1 - controllerConfig.stripeProps.shrinkPct);
					virtualStripe.depth = clone.depth;
					virtualStripe.position = { ...clone.position }; // clone position object
					isRight
						? (virtualStripe.position.x -=
								controllerConfig.stripeProps.stripesOffset)
						: (virtualStripe.position.x +=
								controllerConfig.stripeProps.stripesOffset);
					break;
				}
				...
				case "left": {
					...
					virtualStripe.depth =
						clone.depth * (1 - controllerConfig.stripeProps.shrinkPct);
					virtualStripe.position = { ...clone.position };
					isRight
						? (virtualStripe.position.z +=
								controllerConfig.stripeProps.stripesOffset)
						: (virtualStripe.position.z -=
								controllerConfig.stripeProps.stripesOffset);
					break;
		...
		function calculateIntersectionMidpointOfTwoStripes(
			virtualStripeOfCurrent,
			virtualStripeOfRef,
		) {
			...
			const intersectionMidpointX =
				(intersectionLeftX + intersectionRightX) / 2;
			const intersectionMidpointZ =
				(intersectionTopZ + intersectionBottomZ) / 2;
			const intersectionMidpoint = {
				x: intersectionMidpointX,
				z: intersectionMidpointZ,
			};
		...
		function addSectionDistrictIntersections() {
			glbRelatedRoadObjsMap.forEach((roadObj, _) => {
				...
				for (let i = 0; i < arrLen; i++) {
					...
					if (i === 0) {
						intersectionWithStartBorder = getDistrictIntersection(
							roadObj,
							roadObj.roadSectionObjArr[i],
							(isEnd = false),
						);
						roadObj.roadSectionObjArr[i].intersectionWithStartBorder =
							intersectionWithStartBorder;
					} else if (i === arrLen - 1) {
						intersectionWithEndBorder = getDistrictIntersection(
							roadObj,
							roadObj.roadSectionObjArr[i],
							(isEnd = true),
						);
						roadObj.roadSectionObjArr[i].intersectionWithEndBorder =
							intersectionWithEndBorder;
					}
				}
			});
		}
		...
		function getDistrictIntersection(roadObj, roadSectionObj, isEnd) {
			...
			const virtualDistrictIntersection = calculateDistrictIntersection(
				virtualStripe,
				isEnd,
			);
		...
		function drawSphereAndStoreId(color, x, z) {
			...
			const geometry = new THREE.SphereGeometry(
				controllerConfig.stripeProps.sphereRadius,
				32,
				32,
			);
		...
		function connectCurveIntersections(roadObj) {
			...
			const tubeMaterial = new THREE.MeshBasicMaterial({
				color: getColorForSide(isRight),
			});

			const intersections = roadObj.roadSectionObjArr.filter(
				(roadSectionObj) => roadSectionObj.intersection != null,
			);
			...
			for (let i = 1; i < intersections.length; i++) {
				...
				const tubeGeometry = new THREE.TubeGeometry(
					lineCurve,
					64,
					controllerConfig.stripeProps.tubeRadius,
					8,
					false,
				);
